...
with open("movies_data.csv", encoding="cp1250") as f:
    ...
    for row in reader:
        ...
        cur.execute(
            "INSERT INTO movies (movie_id, title, year_of_production, time, description) VALUES (%s, %s, %s, %s, %s)",
            row[:5]  # upewnij się, że bierzesz tylko tyle kolumn, ile jest w tabeli
        )
